function generate_report() {
    ...
    var data = {
        type_expenses: $('#type_expenses').val(),
        filter: $filter,
        year: $('#year').val(),
        month: $month,
            ...
            },
            {
                ...
                customize: function (doc) {
                    ...
                    doc.content.splice(3, 0, {
                        alignment: 'center',
                        text: ('Reporte de Gastos ' + $report_text).toUpperCase(),
                        style: 'subheader'
                    });
                    doc.content[4].table.widths = ['5%', '15%', '55%', '15%', '10%'];
...
$(function () {
    ...
    $('#filter').change(function () {
        ...
        $('#f_year').hide();
        $('#f_month').hide();
        $('#f_type_expenses').hide();
        table.clear().draw();
        if (filter) {
            $('#f_type_expenses').show();
            switch (filter) {
                case '1':
                    $('#f_start_date').show();
                    $('#f_end_date').show();
    ...
    $('#type_expenses, #month').on('change',function () {
        generate_report();
    });
